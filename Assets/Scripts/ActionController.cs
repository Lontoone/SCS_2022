...
public class ActionController : MonoBehaviour
{
    ...
    IEnumerator DoProcess()
    {
        ...
        {
            if (actionQueue[0].is_in_gap_time_lock && actionQueue.Count > 1)
            {
                actionQueue.RemoveAt(0);
                Debug.Log(actionQueue[0].description + "remove " + actionQueue.Count + " " + (actionQueue.Count > 0));
    ...
    {
        /* ???NOTE??? [允許一個action重覆排隊，但執行到它時若還在冷卻則直接刪除]
        ...
        if (currentAction != null && cDoProcess != null &&
            ...
            _newAct.force)
        {
            Debug.Log(currentAction.description + " 斷 " + _newAct.description);
            //先終止，加入後再重啟
